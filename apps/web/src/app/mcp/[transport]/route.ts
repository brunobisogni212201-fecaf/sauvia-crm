  ...
  server.tool(
    ...
    async (params: any, { authInfo }: any) => {
      try {
        ...
        const client = createBackendClient()
        const response = await client.getPatientAdherence(token, params?.patient_id)
  ...
  server.tool(
    ...
    async (params: any, { authInfo }: any) => {
      try {
        ...
        const client = createBackendClient()
        const response = await client.listPatientPlans(token, params?.patient_id, {
  ...
  server.tool(
    ...
    'Get routine activity logs for a patient',
    {
      ...
      properties: {
        ...
        type: {
          type: 'string',
          description: 'Filter by activity type (MEAL, EXERCISE, MEDICATION, WATER, HABIT)',
        },
      },
      required: ['patient_id'],
    },
    async (params: any, { authInfo }: any) => {
      try {
        ...
        const client = createBackendClient()
        const response = await client.getRoutineLogs(token, params?.patient_id, {
          days: params?.days || 7,
          type: params?.type,
        })
